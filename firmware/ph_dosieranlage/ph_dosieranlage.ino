/*
  ...
  ----------------------------------
  Hardware : ESP32-C3 Super Mini + ADS1115 + pH-Signalboard + TMC2209 + NEMA17
  Board    : esp32:esp32:nologo_esp32c3_super_mini
  Bibliotheken: keine externen - nur der ESP32-Arduino-Core
...
static void ledTask() {
  ...
  switch (controller.state()) {
    case ST_DOSING: period = 100;  break;   // schnelles Blinken = Pumpe laeuft
    case ST_FAULT:  period = 150;  break;
    case ST_LOCKED: period = 1200; break;
    default:        period = 2000; break;   // langsames Lebenszeichen
  }
  if (millis() - last < period) return;
  last = millis();
  phase ^= 1;

  if (controller.state() == ST_FAULT) {
    // Doppelblitz bei Stoerung
    ledWrite(true); delay(40); ledWrite(false); delay(80);
    ledWrite(true); delay(40); ledWrite(false);
  } else {
    ledWrite(phase);
  }
}
...
// ---------------------------------------------------------------------------
// Serielle Konsole - fuer die Inbetriebnahmephasen 1 bis 3 ohne WLAN
// ---------------------------------------------------------------------------
static void i2cScan() {
  Serial.println("[I2C] Scan auf SDA=" + String(PIN_I2C_SDA) + " SCL=" + String(PIN_I2C_SCL));
  uint8_t found = 0;
...
static void printHelp() {
  Serial.println(F(
    ...
    "  mon [n]               n Sekunden Rohwerte im Sekundentakt ausgeben\n"
    ...
    "  stop                  Pumpe anhalten\n"
    "  estop                 Not-Halt (Automatik aus)\n"
    "  clear                 Stoerung/Not-Halt quittieren\n"
    "  cal <a|b> <ph>        Kalibrierpunkt mit aktueller Spannung speichern\n"
    "  calreset              Kalibrierung verwerfen\n"
    "  set <key> <wert>      Parameter setzen (sp db dose maxs maxd pause\n"
    "                        phlock phmax spml sprev srate sacc gain\n"
    "                        invdir hold flowreq flowinv)\n"
    "  wifi <ssid> <pass>    WLAN speichern und neu starten\n"
    "  daily reset           Tageszaehler zuruecksetzen\n"
    "  reboot                Neustart\n"
    "  factory               Werkseinstellungen (loescht alles!)\n"));
}
...
static void printStatus() {
  ...
  Serial.printf("Pumpe: %s (%lu/%lu Schritte) | %.1f Schritte/ml\n",
...
static void handleSet(const String &key, const String &val) {
  ...
  else if (key == "pause")   s.pauseS      = (uint32_t)val.toInt();
  ...
  else if (key == "gain")    s.adcGain     = (uint8_t)val.toInt();
...
static void serialTask() {
  ...
  while (Serial.available()) {
    ...
    if (c == '\r') continue;
    if (c == '\n') {
      if (monUntil) { monUntil = 0; Serial.println("Rohwertausgabe beendet"); buf = ""; continue; }
      handleCommand(buf);
      buf = "";
      Serial.print("> ");
...
void setup() {
  ...
  // Sicherheitsnetz: nach einem Neustart nie sofort losdosieren.
